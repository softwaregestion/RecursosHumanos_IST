...
namespace IST.Api.Controllers
{
    ...
    public class UserOffController : ControllerBase
    {
        ...
        public IActionResult Get(string id)
        {
            ...
            var user = this.context.Query(new RRHH.Domain.Query.AspNetUsers.Details.GetSingleItemByUserName.Query() { Username = id });
            if (user == null)
            {
            ...
            else
            {
                ...
                {
                    var userc = this.context.Query(new RRHH.Domain.Query.AspNetUserClaims.Index.GetAllByUserId.Query() { UserId = user.Id });
