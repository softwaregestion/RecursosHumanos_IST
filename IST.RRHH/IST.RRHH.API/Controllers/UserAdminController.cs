...
namespace IST.RRHH.API.Controllers
{
    ...
    public class UserAdminController : ControllerBase
    {
        ...
        {
            var solicitantes = this.Context.Query(new Domain.Query.AspNetUserClaims.Index.GetAll.Query() {  });
            if (solicitantes.Count > 0)
            {
                ...
                var model = this.Context.Query(new Domain.Query.AspNetUsers.Index.GetAllByIds.Query() { Ids = ids }).Select(c => new UserIndexModel()
                {
                    Email = c.Email,
                    UserId = c.Id,
                    UserName = c.UserName,
