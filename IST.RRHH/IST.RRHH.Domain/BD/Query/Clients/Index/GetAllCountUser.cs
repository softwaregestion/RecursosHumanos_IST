...
{   
	...
	public class GetAllCountUser
	{
		...
		public class Result
		{
			...
			public int CuantosRoles { get; set; }
			public int CuantosUsuarios { get; set; }
		}
        ...
        public class Handler : IQueryHandler<Query, List<Result>>
        {
            ...
            public List<Result> Handle(Query query)
            {
				...
				var rol = context.ClientClaims.ToList();
				var claims = context.AspNetUserClaims.ToList();
				...
				var orderedResults = queriedItem.Select(c=> new Result { Id = c.Id , 
											   ...
											   CuantosUsuarios = Usuarios(c, claims),
												CuantosRoles = Roles(c, rol)
            ...
            private int Roles(Model.Clients cliente, List<Model.ClientClaims> claims)
            {
				return claims.Count(c =>  c.ClientId == cliente.Id && c.Type == "Rol");
            }

			private int Usuarios(Model.Clients cliente, List<Model.AspNetUserClaims> claims)
			{
				return claims.Count(c => c.ClaimType == "application" && c.ClaimValue == cliente.ClientName);
			}
		}
    }
}
