...
namespace IST.RRHH.Domain.Query.TipoNotificacion.Index
{   
	...
	public class GetAll
    {
        ...
        public class Handler : IQueryHandler<Query, List<Result>>
        {
            ...
            public List<Result> Handle(Query query)
            {
                ...
                var queriedItem = context.TipoNotificacion.ToList();

				var orderedResults = queriedItem.Select(c=> new Result { TipoNotificacionId = c.TipoNotificacionId , 
											   Nombre = c.Nombre , 
											   ContenidoPlantilla = c.ContenidoPlantilla });				
